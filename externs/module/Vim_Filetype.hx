...
@:native("vim.filetype") extern class Vim_Filetype {
	...
	**/
	extern static function add(filetypes:externs.type.Vim_Filetype_Add_Filetypes):Dynamic;
	/**
		```lua
		...
		@*return* — : Option value
	**/
	extern static function get_option(filetype:String, option:String):haxe.extern.EitherType<String, haxe.extern.EitherType<Bool, Int>>;
	/**
		```lua
		...
		@*return* — A function that modifies buffer state when called (for example, to set some
		                     ...
		                     its only argument.
	**/
	extern static function match(args:externs.type.Vim_Filetype_Match_Args):vim._internal.Multireturn<haxe.extern.EitherType<String, Void>, haxe.extern.EitherType<haxe.Constraints.Function, Void>>;
}