...
@:luaRequire("vim.lsp.rpc") extern class Vim_Lsp_Rpc {
	/**
		...
		---
		 ...
		 @nodoc
	**/
	extern var client_errors : haxe.extern.EitherType<lua.Table<String, Int>, lua.Table<Int, String>>;
	/**
		```lua
	...
	**/
	extern static function connect(host_or_path:String, port:Null<Int>):(dispatchers:externs.type.Vim_Lsp_Rpc_Dispatchers) -> externs.type.Vim_Lsp_Rpc_PublicClient;
	/**
		```lua
		...
		```
	**/
	extern private static function create_read_loop(handle_body:(body:String) -> Dynamic, ?on_exit:() -> Dynamic, on_error:(err:Any) -> Dynamic):Dynamic;
	/**
		```lua
	...
	**/
	extern static function format_rpc_error(err:lua.Table.AnyTable):String;
	/**
		```lua
	...
	**/
	extern static function rpc_response_error(code:Int, ?message:String, ?data:Any):externs.type.Lsp_ResponseError;
	/**
		```lua
	...
	**/
	extern static function start(cmd:Array<String>, ?dispatchers:externs.type.Vim_Lsp_Rpc_Dispatchers, ?extra_spawn_params:externs.type.Vim_Lsp_Rpc_ExtraSpawnParams):externs.type.Vim_Lsp_Rpc_PublicClient;
}